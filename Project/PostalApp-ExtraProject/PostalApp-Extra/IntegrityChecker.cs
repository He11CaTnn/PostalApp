...
namespace PostalApp_Extra
{
    ...
    // ===================================================================
    // Класс полной проверки целостности файлов PostalApp-Extra
    //
    // ИСПОЛЬЗОВАНИЕ:
    ...
    //
    // Или напрямую:
    //   var report = await IntegrityChecker.CheckAllFiles("0.30", progress);
    // ===================================================================
    ...
    public static class IntegrityChecker
    {
        ...
        /// </summary>
        /// <param name="version">Версия приложения, например "0.30"</param>
            ...
            IProgress<IntegrityProgress> progress = null)
        {
            ...
            try
            {
                entries = await UpdateManagerExtra.GetVersionManifest(version);
            }
            catch (Exception ex)
            {
                throw new Exception($"Не удалось загрузить версионный манифест для v{version}", ex);
            }
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                string actualMd5 = await Task.Run(
                    () => UpdateManagerExtra.CalculateMD5(localPath));
            ...
            return new IntegrityReport(results);
        }
    }
}
