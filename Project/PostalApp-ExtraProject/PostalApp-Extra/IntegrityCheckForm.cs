...
{
    /// <summary>
    /// Форма полной проверки целостности файлов PostalApp-Extra.
    ...
    public partial class IntegrityCheckForm : Form
    {
        ...
        // =================================================================
        // OnLoad — запускает проверку автоматически
        // =================================================================
        ...
        // =================================================================
        // OnShown — выводим форму на передний план
        // =================================================================
        ...
        // =================================================================
        // ГЛАВНАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ ПРОВЕРКИ
        // =================================================================

        private async Task RunIntegrityCheck()
        {
            string currentVersion = UpdateManagerExtra.GetCurrentVersion();
            ...
            try
            {
                await UpdateManagerExtra.GetVersionManifest(currentVersion);
            }
            catch
            {
            ...
            try
            {
                _updateInfo = await UpdateManagerExtra.CheckForUpdates();
            }
            catch
            {
                _updateInfo = null;
        ...
        private void ShowResults(bool manifestChecked)
        {
            ...
            //  B: ошибки + нет обновления       → [Восстановить файлы]
            ...
            else if (updateAvailable)
            {
                ...
                ConfigureButton(_btnAction2, "✕  Закрыть",
                    Color.FromArgb(60, 80, 140), () => { Close(); return Task.CompletedTask; });
                CenterTwoButtons();
            }
            else
            {
                ...
                ConfigureButton(_btnAction1, "✓  Закрыть",
                    Color.FromArgb(30, 140, 70), () => { Close(); return Task.CompletedTask; });
                CenterOneButton(_btnAction1);
                _btnAction2.Visible = false;
            }
        }
        ...
        private async Task DoUpdate()
        {
            ...
            bool downloaded = await UpdateManagerExtra.DownloadUpdate(_updateInfo, progress);
            if (!downloaded)
            {
        ...
        private async Task DoReinstallCurrent()
        {
            ...
            string downloadUrl = $"http://81.90.25.60/extra-updates/versions/v{version}/update.zip";
            ...
            bool downloaded = await UpdateManagerExtra.DownloadUpdate(tempInfo, progress);
            if (!downloaded)
            {
        ...
        {
            btn.Location = new Point((ClientSize.Width - btn.Width) / 2, 418);
            btn.Visible  = true;
        }
