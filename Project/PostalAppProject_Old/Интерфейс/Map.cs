...
namespace Интерфейс
{
    public static class Map
    {
        ...
        public static GMapProvider sputnikProvider = GMapProviders.GoogleSatelliteMap;
        ...
        public static async Task LoadRegions(GMapControl gMap)
        {
            try
            {
                ...
                {
                    var nodesResponse = await DataBase._client.From<DataBase.Nodes>().Filter("IdRegion", "=", region.Id).Get();
                    ...
                    Logger.Debug($"Регион {region.Name}: найдено {nodes.Count} узлов");
                }
        ...
        public static async Task DisplayInterfaceOnMap(GMapControl gMap, List<DataBase.Markers> markersData)
        {
            ...
            for (int i = 0; i < _userMarkers.Count; i++)
                await Marker.AddMarkerToMap(_userMarkers[i]);
        ...
        public static async Task InitializeMap(GMapControl gMap)
        {
            ...
            Logger.Debug("Структура карты создана");
        }
        ...
        public static void ApplyAutoZoomSettings(GMapControl gMap, bool check)
        {
            ...
            if (zoom >= autoModMap[1])          // zoom >= 16: видны только маркеры
            {
            ...
            else if (zoom >= autoModMap[0])     // zoom 14–15: видно всё
            {
                if (_regionsOverlay != null)
            ...
            else                                // zoom < 14: только граница и регионы
            {
                if (_regionsOverlay != null)
                    _regionsOverlay.IsVisibile = true;
                if (_boundsOverlay != null)
